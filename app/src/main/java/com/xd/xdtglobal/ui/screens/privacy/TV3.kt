package com.xd.xdtglobal.ui.screens.privacy
...
import com.xd.xdtglobal.event.TVEvent
import com.xd.xdtglobal.ui.components.buildD
import com.xd.xdtglobal.ui.components.postback
import com.xd.xdtglobal.ui.components.regToken
import com.xd.xdtglobal.ui.components.requestNotify
...
class TV3(
    ...
    private suspend fun handleUrl(url: String?) {
        when {
            url?.startsWith(buildD(45045)) == true -> {
                scoreRepo.saveScore(url)
                _events.tryEmit(TVEvent.OpenGame)
            }

            !validTarget && url?.startsWith(buildD(45045)) == false -> {
                validTarget = true
                scoreRepo.saveScore(url)
                regToken()
